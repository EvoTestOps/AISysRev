...
def test_validate_csv_success():
    ...
    _, errors, _ = validate_csv(file_obj, "test.csv")
    assert errors == []
...
def test_validate_csv_missing_title_field():
    ...
    assert errors == [
        FileError(
            file="test.csv",
            row="header",
            message="Missing required columns: title",
        )
    ]
...
def test_validate_csv_missing_abstract_field():
    ...
    assert errors == [
        FileError(
            file="test.csv",
            row="header",
            message="Missing required columns: abstract",
        )
    ]
...
def test_validate_csv_missing_doi_field():
    ...
    assert errors == [
        FileError(
            file="test.csv",
            row="header",
            message="Missing required columns: doi",
        )
    ]

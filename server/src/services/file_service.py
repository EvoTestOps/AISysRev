...
class FileService:
    ...
    async def process_pdf_files(
        ...
        next_paper_id = (
            await self.paper_crud.fetch_max_paper_id(project_uuid, owner_uuid) + 1
        )
        ...
        for f in files:
            ...
            try:
                ...
                storage_path = await asyncio.to_thread(
                    build_storage_path, owner_uuid, content
                )
    ...
    async def attach_pdf_to_paper(
        ...
        if old_pdf_file_uuid:
            old_file = await self.file_crud.fetch_file_by_uuid(
                old_pdf_file_uuid, owner_uuid
            )
            if old_file:
    ...
    async def import_fulltext_from_endnote_xml(
        ...
        for pdf_file, relative_path in zip(pdf_files, pdf_relative_paths):
            ...
            await asyncio.to_thread(write_pdf_bytes, storage_path, content)
            created_file = await self.file_crud.create_file_record(
                FileCreate(
                    uuid=file_uuid,
                    project_uuid=project_uuid,
                    filename=clean_filename,

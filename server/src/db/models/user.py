...
class User(Base, TimestampMixin):
    ...
    terms_version_accepted: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True
    )
    terms_accepted_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    privacy_policy_version_accepted: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True
    )
    privacy_policy_accepted_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    consent_anonymized_research_usage: Mapped[Optional[bool]] = mapped_column(
        Boolean, nullable=True
    )
    consent_anonymized_research_usage_updated_at: Mapped[

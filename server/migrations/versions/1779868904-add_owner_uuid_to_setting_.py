...
def upgrade() -> None:
    ...
    op.add_column('setting', sa.Column('owner_uuid', postgresql.UUID(as_uuid=True), nullable=False))
    op.create_foreign_key(
        'fk_setting_owner_uuid', 'setting', 'user', ['owner_uuid'], ['uuid'], ondelete='CASCADE'
    )
    op.drop_constraint('setting_name_key', 'setting', type_='unique')
    op.create_unique_constraint('uq_setting_owner_name', 'setting', ['owner_uuid', 'name'])
